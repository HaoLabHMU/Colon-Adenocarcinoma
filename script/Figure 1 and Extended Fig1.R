...
#----------------------------------------------------------------------------------------------------------
#### Fig.1B and Extended Data Fig.1E  ####
#----------------------------------------------------------------------------------------------------------
#Fig.1B
...
pdf("./umap by cellType_1.pdf",width = 6,height=6)
print(p1)
dev.off()

#Extended Data Fig.1E
load("CellMetaData.rda")
object@meta.data <- CellMetaData
my.colors <- c("Yes"="#d62728","None" = "#c7c7c7")
p2 <- DimPlot(object, reduction = "umap",label =F,group.by = "TCR",
              pt.size = 1,raster=T,shuffle=T,cols=my.colors)

my.colors <- c("Yes"="#2171b5","None" = "#c7c7c7")
p3 <- DimPlot(object, reduction = "umap",label =F,group.by = "BCR",
              pt.size = 1,raster=T,shuffle=T,cols=my.colors)
...
for (i in unique(metaData_H$Tissue2)) {
  ...
  plot.path=paste0("./umap by Density of ",i,"_healthy.pdf")
...
for (i in unique(metaData_CA$Tissue2)) {
  ...
  plot.path=paste0("./umap by Density of ",i,"_CA.pdf")
  ggsave(plot.path,width = 6,g.overlay)
}
#----------------------------------------------------------------------------------------------------------
#### Fig.1D ####
...
#Figure 1H:
BCellMetaData <- CellMetaData[CellMetaData$cellType_1 %in% c("B cells","Plasma"),]
df <- clone_mutation[clone_mutation$sequence_id %in% BCellMetaData$cells,]
df$cellType <- BCellMetaData[df$sequence_id,"CellType_n"]
...
p1 <- ggplot(data =  df2, aes(x = Tissue, y = cellType)) + 
  ...
  scale_fill_gradientn(colours=c("#4393c3","#92c5de","#f7f7f7","#f4a582","#d6604d","#b30000"),
                       ...
                       guide="colorbar")
#----------------------------------------------------------------------------------------------------------
#### Extended Data Fig.1F & 1I ####
#----------------------------------------------------------------------------------------------------------
Tcell <- subset(object, subset=cellType_1=="T cells")

#1.  clustering of T cells
{
  TCR.genes <- grep("^TR[AB][VJ]",rownames(Tcell),value = T)
  Tcell@assays$RNA@scale.data <- matrix()
  Tcell <- NormalizeData(Tcell)
  Tcell <- FindVariableFeatures(Tcell,nfeatures = 2000)
  var.genes <- VariableFeatures(Tcell)
  var.genes <- setdiff(var.genes,TCR.genes)
  VariableFeatures(Tcell) <- var.genes
  Tcell <- ScaleData(Tcell,features = var.genes)
  Tcell <- RunPCA(Tcell, verbose = FALSE,features = VariableFeatures(Tcell),npcs=30)
  Tcell <- RunHarmony(Tcell, group.by.vars="orig.ident",assay.use ="RNA")
  Tcell <- FindNeighbors(Tcell, dims=1:30,reduction = "harmony",k.param = 30)
  Tcell <- FindClusters(Tcell,resolution=1,random.seed=123,graph.name = 'RNA_snn')
  Tcell <- RunUMAP(Tcell,reduction = "harmony",seed.use = 123,dims=1:30,
                   umap.method='uwot',min.dist=0.2,spread=1)
}

#2. Feature plot split by tissues
p1 <- FeaturePlot(Tcell, features = "CD160",
                  raster=T,reduction = "umap",
                  pt.size = 2,split.by = "Tissue2")
#----------------------------------------------------------------------------------------------------------
