...
#### Figure 2C ####
#----------------------------------------------------------------------------------------------------------
#1. Favorable prognostic genes from TCGA-CRC dataset
{
  library(survival)
  CoxDF <- data.frame(genes=genes,HR=NA,P=NA,row.names = genes)
  for (i in genes) {
    exp.i <- TG_exp[i,rownames(CRC_clin)]
    if (sd(exp.i) > 0) {
      exp.i <- scale(exp.i)[,1]
      cox.i <- coxph(Surv(DFI.time, DFI) ~ exp.i, CRC_clin)
      cox.i <- summary(cox.i)
      CoxDF[i,"HR"] <- cox.i$coefficients[,2]
      CoxDF[i,"P"] <- cox.i$coefficients[,5]
    }
  }
  prog.genes <- CoxDF$genes[which(CoxDF$HR<1 & CoxDF$P < 0.05)]
  CoxDF <- CoxDF[prog.genes,]
  write.table(CoxDF,file = "./Prognostic genes in TCGA.txt",
              sep = "\t",quote = F,row.names = F)
}

#2. average expression of prognostic genes across T cell types
{
  Tcell <- subset(object, subset=cellType_1=="T cells")
  prog.genes <- fread("./Prognostic genes in TCGA.txt")
  iOrd <- intersect(prog.genes$Genes,rownames(Tcell))
  iOrd <- colMeans(GetAssayData(Tcell)[iOrd,])
  Tcell$prog.genes <- iOrd
  df <- Tcell@meta.data
  df$CellType_n <- metaData_fil[rownames(df),"CellType_n"]
  df$cellType_2 <- metaData_fil[rownames(df),"cellType_2"]
  df$cellType_2[df$CellType_n %in% c("Induced.IEL","Natural.IEL")] <- df$CellType_n[df$CellType_n %in% c("Induced.IEL","Natural.IEL")]
  df <- df[df$Tissue2 == "T",]
  iOrd <- c("CD4_Naive","CD4_Tcm","CD8_Mem","GZMK+ effector","CD4_Tem","Natural.IEL",
            "Induced.IEL","Tfh","Treg")
  df <- df[df$CellType_n %in% iOrd,]
  ggplot(df,aes(CellType_n, prog.genes)) + 
    geom_boxplot(aes(fill = CellType_n),outlier.shape = NA,alpha = 1)+
    scale_fill_manual(values=all.cols)+
    theme_classic()+ylab("prog.genes")+
    theme(legend.position="none",axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
}

#3.  average expression of T cell activation genes across T cell types:
{
  Tact <- fread("./T_activation.txt")
  iOrd <- colMeans(GetAssayData(Tcell)[Tact$Gene,])
  Tcell$Tact <- iOrd
  df <- Tcell@meta.data
  df$CellType_n <- metaData_fil[rownames(df),"CellType_n"]
  df$cellType_2 <- metaData_fil[rownames(df),"cellType_2"]
  df$cellType_2[df$CellType_n %in% c("Induced.IEL","Natural.IEL")] <- df$CellType_n[df$CellType_n %in% c("Induced.IEL","Natural.IEL")]
  df <- df[df$Tissue2 == "T",]
  iOrd <- c("CD4_Naive","CD4_Tcm","CD8_Mem","GZMK+ effector","CD4_Tem",
            "Induced.IEL","Tfh","Treg")
  df <- df[df$CellType_n %in% iOrd,]
  ggplot(df,aes(CellType_n, Tact)) + 
    geom_boxplot(aes(fill = CellType_n),outlier.shape = NA,alpha = 1)+
    scale_fill_manual(values=all.cols)+
    theme_classic()+ylab("Tact")+
    theme(legend.position="none",axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
}

#4. average expression of T cell Dysfuncion genes across T cell types:
{
  Dysfunction <- c("PDCD1","HAVCR2","LAG3","CD74","TNFRSF18","LAYN","TIGIT","ICOS","CTLA4","LYST")
  iOrd <- colMeans(GetAssayData(Tcell)[Dysfunction,])
  Tcell$Dysfunction <- iOrd
  df <- Tcell@meta.data
  df$CellType_n <- metaData_fil[rownames(df),"CellType_n"]
  df$cellType_2 <- metaData_fil[rownames(df),"cellType_2"]
  df$cellType_2[df$CellType_n %in% c("Induced.IEL","Natural.IEL")] <- df$CellType_n[df$CellType_n %in% c("Induced.IEL","Natural.IEL")]
  ...
  iOrd <- c("CD4_Naive","CD4_Tcm","CD8_Mem","GZMK+ effector","CD4_Tem",
            "Induced.IEL","Tfh","Treg")
  df <- df[df$CellType_n %in% iOrd,]
  ggplot(df,aes(CellType_n, Dysfunction)) + 
    geom_boxplot(aes(fill = CellType_n),outlier.shape = NA,alpha = 1)+
    scale_fill_manual(values=all.cols)+
    theme_classic()+ylab("Dysfunction")+
    theme(legend.position="none",axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
}
#----------------------------------------------------------------------------------------------------------
#### Figure 2D ####
#----------------------------------------------------------------------------------------------------------
#1. upregulated genes in Responders
{
  # download the DEGs list from the supplementray files of "PMID: 37563240"
  DEGs <- fread("./GSE235919/DEGs in Fig3b.txt")
  DEGs <- data.frame(DEGs,row.names = DEGs$Gene)
  upDEGs <- DEGs$Gene[which(DEGs$log2FC>0 & DEGs$log10P> -log10(0.05))]
}

#2. expression upDEGs across cell types of tumor T cells:
{
  iOrd <- intersect(upDEGs,rownames(Tcell))
  Tcell$upDEGs <- colMeans(GetAssayData(Tcell)[iOrd,])
  {
    df <- Tcell@meta.data
    df <- df[df$Tissue2 == "T",]
    iOrd <- c("CD4_Naive","CD4_Tcm","CD8_Mem","GZMK+ effector","CD4_Tem",
              "Induced.IEL","Tfh","Treg")
    df <- df[df$CellType_n %in% iOrd,]
    p1 <- ggplot(df,aes(CellType_n, upDEGs)) + 
      geom_boxplot(aes(fill = CellType_n),outlier.shape = NA,alpha = 1)+
      scale_fill_manual(values=all.cols)+
      theme_classic()+ylab("upDEGs")+
      theme(legend.position="none",axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
  }
}
#----------------------------------------------------------------------------------------------------------
#### Figure 2F ####
#----------------------------------------------------------------------------------------------------------
#1. calculate progenitor exhaustion score for T cells:
{
...
#2. Progenitor and terminal exhaustion score for induced_IELs (TCR alpha beta+).
{
  load("scores.rda")
  df <- object@meta.data
  df <- df[df$CellType_n=="Induced.IEL",]
  df$Tissue2 <- factor(df$Tissue2,levels = names(Tissue.colors))
  df$Tex_score <- scores[df$cells,"Exh.score"]
  df$Pex_score <- Prog.scores[df$cells]
  
  p1 <- ggplot(df,aes(Tissue2, Pex_score)) + 
    geom_boxplot(aes(fill = Tissue2),outlier.shape = NA,alpha = 1)+
    scale_fill_manual(values=Tissue.colors)+
    theme_classic()+ylab("Exh_Pex score")+
    theme(legend.position="none",axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
    facet_wrap(~Source)
  
  p2 <- ggplot(df,aes(Tissue2, Tex_score)) + 
    geom_boxplot(aes(fill = Tissue2),outlier.shape = NA,alpha = 1)+
    scale_fill_manual(values=Tissue.colors)+
    theme_classic()+ylab("Exh_Tex score")+
    theme(legend.position="none",axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
    facet_wrap(~Source)
}

#3. CYT for gamma delta T.
{
  load("scores.rda")
  df <- object@meta.data
  df <- df[df$CellType_n=="Natural.IEL",]
  df$Tissue2 <- factor(df$Tissue2,levels = names(Tissue.colors))
  df$CYT <- scores[df$cells,"Cyt.score"]
  
  p1 <- ggplot(df,aes(Tissue2, CYT)) + 
    geom_boxplot(aes(fill = Tissue2),outlier.shape = NA,alpha = 1)+
    scale_fill_manual(values=Tissue.colors)+
